...
public class DFSInMatrixRecursion {
    ...
    public void DFSUtil(int[][] grid, int row, int col, boolean[][] visited) {
        ...
        DFSUtil(grid, row+ 1, col,visited); // go down
       DFSUtil(grid, row- 1, col,visited); //go  up
        DFSUtil(grid, row, col + 1,visited); //go right
        DFSUtil(grid, row, col - 1,visited); // go left
    }
